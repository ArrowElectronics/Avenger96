...
{	
	...
	then
		cp -r $YOCTO_DIR/Avenger96_L5_10_74_Rel_1_3_patches/meta-einfochips-ap1302/ .
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
			echo "${red}Error during apply the patch in meta-einfochips-ap1302"
			echo "Please verify Avenger96_L5_10_74_Rel_1_3_patches directory${reset}"
			echo "###################################################################################"
			exit 1
		fi
	fi
	...
	git apply --check -R $YOCTO_DIR/Avenger96_L5_10_74_Rel_1_3_patches/meta-dhsom-stm32-common-patches/00* 2>/dev/null
	if [ $? -ne 0 ]
	then
		echo "###################################################################################"
		...
		git am --whitespace=fix $YOCTO_DIR/Avenger96_L5_10_74_Rel_1_3_patches/meta-dhsom-stm32-common-patches/00*
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
	...
	git apply --check -R $YOCTO_DIR/Avenger96_L5_10_74_Rel_1_3_patches/meta-dhsom-stm32-bsp-patches/00* 2>/dev/null
	if [ $? -ne 0 ]
	then
		echo "###################################################################################"
		echo "Apply the patch in meta-dhsom-stm32-common"
		...
		git am --whitespace=fix $YOCTO_DIR/Avenger96_L5_10_74_Rel_1_3_patches/meta-dhsom-stm32-bsp-patches/00*
		if [ $? -ne 0 ]
		then
			echo "###################################################################################"
